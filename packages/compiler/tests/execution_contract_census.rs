...
/// Semantic collection is an observational side channel. Across the complete
/// fixture and adversarial-probe corpus it may change only the returned trace,
/// never code, source maps, or diagnostics.
#[test]
fn semantic_trace_is_corpus_wide_output_neutral() {
    let mut plain_options = options(false);
    plain_options.source_map = true;
    let mut traced_options = options(true);
