...
#[test]
fn ssr_lone_spread_records_discarded_native_refs() {
    for source in [
        "let node; const view = <div ref={node} {...props()} />;",
        "const view = <div {...props()} ref={factory()} />;",
        "const view = <div {...props()} ref={factory()} id={id()} />;",
    ] {
        let settings = CompileOptions {
            generate: Generate::Ssr,
            source_map: true,
            ..options(true)
        };
        let traced = compile(source, &settings).unwrap();
        let plain = compile(
            source,
            &CompileOptions {
        ...
        assert_eq!(traced.source_map, plain.source_map);
        let trace = traced.semantic_trace.unwrap();
        let site = trace
            .sites
            .iter()
            .find(|site| site.kind == ExecutionSiteKind::Ref)
            .unwrap();
        assert_eq!(
            site.decision,
...
fn trace_v3_binds_identity_and_reports_independent_execution_axes() {
    ...
    assert_eq!(trace.identity.config.patch_driver, "disabled");
...
/// Current Solid source children win over a dynamic `textContent` attribute.
/// The losing attribute remains explicit as elided data while the child takes
/// the reactive insert path.
#[test]
fn source_children_win_over_dynamic_text_content() {
    ...
    assert!(
        rendered.owner_establishments.iter().any(|fact| {
            fact.wrapper == "insert" && source_text(source, fact.span.start, fact.span.end) == "y()"
        }),
        "the surviving source child must report its insert owner"
    );
...
/// Current Solid promotes a native `children` attribute to child content at
/// both template roots and nested native elements. The trace follows the
/// selected lowering path without retaining speculative static-path facts.
#[test]
fn native_children_trace_matches_the_selected_lowering_path() {
    ...
    {
        let source = "const el = <span children={x()} />;";
        let rendered = trace(source);
        assert!(
            rendered.sites.iter().any(|site| {
                source_text(source, site.span.start, site.span.end) == "x()"
    ...
    // A nested promoted value forces dynamic child lowering and reports the
    // emitted insert at the source expression.
    for source in [
        ...
        assert!(
            ...
            "{source}: the nested promoted value is reactive, got {:?}",
            rendered.sites
        );
        assert!(
            rendered.owner_establishments.iter().any(|fact| {
            ...
            "{source}: the nested promoted value reports its insert, got {:?}",
            rendered.owner_establishments
        );
    }
    ...
    // Dynamic attributes or callbacks force full nested lowering without
    // changing promotion of the `children` value.
    for source in [
        ...
        assert!(
            ...
            "{source}: full nested lowering keeps the child reactive, got {:?}",
            rendered.sites
        );
        assert!(
            rendered.owner_establishments.iter().any(|fact| {
            ...
            "{source}: full nested lowering reports the child insert, got {:?}",
            rendered.owner_establishments
        );
    }

    // Solid writes one child slot per element, and the last writer wins: a
    // dynamic `textContent` after `children` discards the promoted value. The
    ...
    // With `children` last, it wins the slot and takes the reactive insert;
    // the preceding `textContent` value remains explicit as elided data.
    ...
    // claims nothing, and the surviving value takes the promoted child path.
